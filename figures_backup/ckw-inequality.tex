\begin{figure}[htbp]
	...
	\begin{tikzpicture}[scale=1.1]
		...
		\node[draw, rounded corners, fill=yellow!20, text width=12cm, align=left, below=0.8cm] at (2,-0.4) {
			\textbf{CKW inequality:} $C_{AB}^2 + C_{AC}^2 \leq C_{A(BC)}^2$ \quad
			\textbf{Tangle:} $\tau = C_{A(BC)}^2 - C_{AB}^2 - C_{AC}^2$ ("excess" genuine tripartite entanglement)
		};
	\end{tikzpicture}
