\begin{figure}[htbp]
	...
	\end{tikzpicture}
	\caption{GHZ state representation: (a) Complete graph $K_4$ requires many pairwise edges. (b) GHZ hypergraph represents the same state with a single hyperedge connecting all $N$ qubits, capturing the genuine N-partite entanglement. Single hyperedge captures $|GHZ_N\rangle = \frac{1}{\sqrt{2}}(|0\rangle^{\otimes N}+|1\rangle^{\otimes N})$.}
