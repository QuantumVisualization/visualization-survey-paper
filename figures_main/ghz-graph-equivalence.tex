\begin{figure}[htbp]
	...
	\vspace{0.3cm}
	\caption{Different graph representations of the GHZ state. The GHZ state can be represented as a path graph, cycle graph, or other connected structures, all of which are locally complemented to each other and thus share equivalent entanglement properties. GHZ state equivalence: $|\text{GHZ}_N\rangle = \frac{1}{\sqrt{2}}(|0\rangle^{\otimes N} + |1\rangle^{\otimes N})$ can be represented by different graph structures that are LC-equivalent (connected through local complementations).}
