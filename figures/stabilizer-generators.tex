\begin{figure}[htbp]
	...
	\end{tikzpicture}
	\caption{Reading stabilizer generators from the graph structure. For each vertex $i$, the stabilizer generator $K_i$ is the product of $X$ on vertex $i$ and $Z$ on all its neighbors. These $N$ operators generate the full stabilizer group of $2^N$ elements. For vertex $i$: $K_i = X_i \prod_{j \in N(i)} Z_j$, where $N(i)$ is the set of neighbors of $i$. The stabilizer group is $\mathcal{S} = \langle K_1, K_2, \ldots, K_N \rangle$.}
