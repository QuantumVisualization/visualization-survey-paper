\begin{figure}[htbp]
	...
	\end{tikzpicture}
	\caption{Illustration of local complementation on a graph state. When LC is applied to vertex $b$, an edge is added between its neighbors $a$ and $c$. States related by LC (and edge additions/deletions) have equivalent entanglement structure. Local Complementation (LC): Applying LC on vertex $v$ adds edges between all pairs of neighbors of $v$ that weren't previously connected.}
