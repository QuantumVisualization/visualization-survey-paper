...
\input{figures_main/dcn-3qubit.tex}
...
The Paulivec visualization, implemented in Qiskit as \texttt{plot\_state\_paulivec}, maps the coherence vector of a quantum state to a collection of Bloch spheres. For an $N$-qubit state, each qubit's reduced density matrix $\rho_i$ is represented by a Bloch sphere, where the Bloch vector $\vec{r}_i = (\langle X_i \rangle, \langle Y_i \rangle, \langle Z_i \rangle)$ is plotted as a point on the sphere. The sphere's interior represents mixed states, with radius proportional to the purity $\text{Tr}(\rho_i^2)$.
